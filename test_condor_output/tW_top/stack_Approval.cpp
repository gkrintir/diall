...
void stack_Approval(const TString& variable, const TString& dilepton, const Int_t nBinsX, const Float_t binXLow, const Float_t binXUp, const TString& condition) {
   ...
   const TString folder = "/home/fynu/gkrintiras/CMSSW_7_5_7_patch2/src/UserCode/diall/test_condor_output/tW_top";
   ...
   for (int i=1; i<2; i++){ //51
     ...
     addToStack(Form("%s/%s_%d.root", folder.Data(),"test", i), variable.Data(), dilepton, "Single_top", true, nBinsX, binXLow, binXUp, kBlack, 21, kBlack, *hs_data );
   ...
   */
   ...
   for (Long64_t i=0;i< nentries;i++) {
     ...
     for (int j = 0; j < px->size(); ++j) {
       //if((*njets)[j]<2)
       if ((*ndilep)[j]>=1 && (*njets)[j]>=2 && (*dilepmass)[j]>=55)
