...
*/
void stack_Approval(const TString& variable, const TString& dilepton, const Int_t nBinsX, const Float_t binXLow, const Float_t binXUp, const TString& condition) {
   ...
   const TString folder = "/home/fynu/gkrintiras/CMSSW_7_5_7_patch2/src/UserCode/diall/test_condor_output/DY/jeteta1";
   THStack *hs_data = new THStack("hs_data","Signal Region");
   
   //addToStack(Form("%s/%s_%s.root", folder.Data(),"data", iso.Data()), "__mtw_array", "Single_top", true, kBlack, 21, kBlack, *hs_data );
   for (int i=1; i<2; i++){ //51
     //if (i==38 || i==37 || i==25) continue;
     addToStack(Form("%s/%s_%d.root", folder.Data(),"testElele", i), variable.Data(), dilepton, "Single_top", true, nBinsX, binXLow, binXUp, kBlack, 21, kBlack, *hs_data );
   ...
   /*
   const TString folder = "Input/lessiso/sr_region/13TeV/NoMTWCut";
   THStack *hs = new THStack("hs","");
   //create three 1-d histograms
   

   addToStack(Form("%s/%s_%s.root", folder.Data(),"tchannel", iso.Data()), "__mtw_array", "Single_top", false, kRed, 21, kRed, *hs );
   ...
   */
   ...
   if (condition.EqualTo(""))hs_data->GetStack()->Last()->SaveAs(Form("%s_%s.root", variable.Data(), dilepton.Data()));
   else hs_data->GetStack()->Last()->SaveAs(Form("%s_%s_%s.root", variable.Data(), condition.Data(), dilepton.Data()));
...
void addToStack(const TString& filename, const TString& variable,  const TString& legentry, Bool_t sumw2, const EColor& fillcolor, Int_t MarkerStyle,  const EColor& MarkerColor, THStack& hs) {
   ...
   TList *anaPFvsCaloJet = dynamic_cast<TList*>(f->Get("anaEMu"));
   TH1F * pt_muon = dynamic_cast<TH1F*>(anaPFvsCaloJet->FindObject(Form("%s",variable.Data())));
...
void addToStack(const TString& filename, const TString& variable, const TString& dilepton, const TString& legentry, Bool_t sumw2, Int_t nbins, Float_t binXLow, Float_t binXUp,
   ...
   TList *anaPFvsCaloJet = dynamic_cast<TList*>(f->Get(Form("%s", dilepton.Data())));
   ...
   /*
   //anaTtbarEMuInfo->SetBranchStatus("fEleEta",1);
   //anaTtbarEMuInfo->SetBranchAddress("fEleEta", &eleeta);
   ...
   */
   ...
   for (Long64_t i=0;i< nentries;i++) {
     ...
     for (int j = 0; j < px->size(); ++j) {
       //if((*njets)[j]<2)
       // if ((*ndilep)[j]>=1 && (*dilepsign)[j]<1 && (*dilepmass)[j]>20 && (*njets)[j]>nNjetsCut && (*elept)[j]> elePtCut )
       //if ((*ndilep)[j]>=1 && (*dilepsign)[j]<1 && (*dilepmass)[j]>20 && (*elept)[j]> elePtCut )
       if ((*ndilep)[j]>=1 && (*dilepsign)[j]<1 && (*elept)[j]> elePtCut )
	 //if((*dilepmass)[j]<80 || (*dilepmass)[j]>100)
